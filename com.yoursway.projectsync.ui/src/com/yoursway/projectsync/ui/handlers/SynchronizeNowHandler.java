...
public class SynchronizeNowHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		if (ProjectSync.getFolders().isEmpty())
			return new ConfigureSynchronizationHandler().execute(event);
				...
				shell.getDisplay().asyncExec(new Runnable() {
					...
					public void run() {
						if (!warnings.isEmpty()) {
							...
							message
									.append("The following warnings were issued during synchronization:\n");
							for (String warning : warnings)
								message.append("• ").append(warning).append(
										"\n");
							MessageDialog.openWarning(shell, "ProjectSync",
									message.toString());
						}
					}
